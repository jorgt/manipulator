...
onmessage = function(oEvent) {
	var opt = oEvent.data;
	_staging = opt.data;
	_data.width = opt.width;
	_data.height = opt.height;
	_data.kmeans(opt.kmeans, []);
	_data.smoothing(opt.smoothing[0], opt.smoothing[1]);
	_data.mergeCells(opt.merge[0], opt.merge[1]);
	postMessage(_staging);
};